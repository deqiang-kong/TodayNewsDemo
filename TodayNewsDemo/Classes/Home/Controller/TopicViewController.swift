...
extension TopicViewController {
    ...
    @objc fileprivate func setRefresh() {
        let header = RefreshHeder(refreshingBlock: { [weak self] in
            ...
            HomeAPI.getCategoryRefreshData(refresh: (self?.refreshFlag)! ,category: self!.topicTitle!.category!) { (nowTime, newsTopics) in
        ...
        tableView.mj_footer = MJRefreshBackNormalFooter(refreshingBlock: { [weak self] in
            ...
            HomeAPI.getHomeCategoryMoreData(category: self!.topicTitle!.category!) { (nowTime, newsTopics) in
...
extension TopicViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    private func getRealVideoURL(weitoutiao: WeiTouTiao) {
        HomeAPI.parseVideoRealURL(video_id: weitoutiao.video_id!) { (realVideo) in
            let videoDetailVC = VideoDetailController()
...
//                navigationController?.pushViewController(questionAnswerVC, animated: true)
            } else if (weitoutiao.has_image != nil && weitoutiao.has_image!) {
                // 说明有图片
    ...
    private func loadNewsDetail(weitoutiao: WeiTouTiao) {
        if let articleURL = weitoutiao.article_url {
            ...
            HomeAPI.getCommenNewsDetail(articleURL: articleURL, completionHandler: { (htmlString, images, abstracts) in
