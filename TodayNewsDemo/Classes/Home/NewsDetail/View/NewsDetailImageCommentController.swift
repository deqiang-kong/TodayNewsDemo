...
class NewsDetailImageCommentController: AnimatableModalViewController, StoryboardLoadable {
    ...
    var weitoutiao: WeiTouTiao? {
        didSet {
            NewsDetailAPI.loadNewsDetailImageComments(offset: 0, item_id: weitoutiao!.item_id!, group_id: weitoutiao!.group_id!) { (comments) in
...
extension NewsDetailImageCommentController {
    ...
    fileprivate func setupUI() {
        ...
        tableView.mj_footer = MJRefreshBackNormalFooter(refreshingBlock: { [weak self] in
            // 获取评论数据
            NewsDetailAPI.loadNewsDetailImageComments(offset: self!.comments.count, item_id: self!.weitoutiao!.item_id!, group_id: self!.weitoutiao!.group_id!) { (comments) in
