...
class VideoDetailController: UIViewController {
    ...
    var videoTopic: WeiTouTiao? {
        didSet {
            ...
            NewsDetailAPI.loadNewsDetailRelateNews(fromCategory: "", weitoutiao: videoTopic!) { (relateNews, labels, userLike, appInfo, filter_wrods) in
                self.relateNews = relateNews
                self.relateTableView.reloadData()
                
                //let videoCode = "74ae6ff946984c239ed8215c4913f03f"
                /// 获取视频的真实链接
                HomeAPI.parseVideoRealURL(video_id: self.videoCode) { (realVideo) in
...
extension VideoDetailController {
    ...
    fileprivate func setupUI() {
        ...
        // 获取评论数据
        NewsDetailAPI.loadNewsDetailComments(offset: offset, weitoutiao: videoTopic!) { (comments) in
        ...
        commentTableView.mj_footer = MJRefreshBackNormalFooter(refreshingBlock: { [weak self] in
            // 获取评论数据
            NewsDetailAPI.loadNewsDetailComments(offset: self!.comments.count, weitoutiao: self!.videoTopic!) { [weak self] (comments) in
...
//                                              definition: "高清")
         ...
         let video = realVideo
        let videoUrl = realVideo!.video_1!.main_url
